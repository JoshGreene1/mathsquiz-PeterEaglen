...
class Login:
    def __init__(self, parent):        
        self.user_details = [["1", "2"],["Steven", "IsCool101"]]
    ...
    def Create_Account(self):    
        count = 0   
        while count < len(self.user_details):
            if self.Create_Username.get() == self.user_details[count][0]:
                print(self.Create_Username.get(), self.user_details[count][0])
                    ...
                    try:    
                        if self.Create_Age.get() >= 8 and self.Create_Age.get() <= 14:
                            if self.Create_Password.get() == self.Confirm_Password.get():
                                self.user_details.insert(0,[self.Create_Username.get(), self.Create_Password.get()])  
                                self.frame1.grid_remove()
    ...
    def ShowMathsQuiz(self):
        ...
        while count < len(self.user_details):
            if self.username_entry.get() in self.user_details[count][0]:
                if self.UsersEntry == self.user_details[count]:
